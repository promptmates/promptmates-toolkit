...
def restart_daemon(name=None):
    """Stop the daemon. A subsequent browsermate call will auto-spawn a fresh one."""
    import signal
...
def start_remote_daemon(name="remote", profileName=None, **create_kwargs):
    ...
    kwargs forwarded to POST /browsers (camelCase):
      profileId, proxyCountryCode, timeout, customProxy,
      browserScreenWidth/Height, allowResizing, enableRecording.
    """
    if daemon_alive(name):
...
def list_local_profiles():
    """Detected local browser profiles. Requires `profile-use` CLI."""
